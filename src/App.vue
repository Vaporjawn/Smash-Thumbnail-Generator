<template>
  <v-app>
    <v-app-bar app dense dark :src="game.bgSrc">
      ...
      <v-tooltip bottom>
        <template v-slot:activator="{ on, attrs }">
          <v-btn icon href="https://github.com/Vaporjawn/Smash-Thumbnail-Generator" target="_blank" v-bind="attrs" v-on="on">
...
export default Vue.extend({
  ...
  methods: {
    ...
    updateGame(id) {
      this.game = this.games.find(g => g.id === id);
    ...
    quickPrevOrNext(prev) {
      ...
      var quickList;
      try {
        quickList = JSON.parse(localStorage.quickList);
      } catch (err) {
        console.error(err);
        localStorage.removeItem('quickList');
        this.hasQuickPrev = false;
        this.hasQuickNext = false;
        this.$toasted.show('Quick list data was invalid and has been reset.', {
          icon: {
