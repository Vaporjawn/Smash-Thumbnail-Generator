...
import smashgg from '../../../utils/rest/smashgg';
...
// TODO _.isEmpty(..)
  ...
  // This component's `computed.show` (a getter/setter pair, for v-model
  // support) combined with the rest of its shape defeats Vue 2.7's
  // Vue.extend() overload resolution - it falls through to the untyped
  // `ComponentOptions<Vue>` overload and every `this.*` access then errors
  // as "does not exist", even though it all works correctly at runtime.
  // Casting the whole options object sidesteps that inference gap without
  // changing any runtime behavior (still passed through Vue.extend() as-is).
} as any);
</script>
