...
import ConfPlayer from "./config/ConfigPlayer.vue";
import ConfText from "./config/ConfigText.vue";
import ConfBackground from "./config/ConfigBackground.vue";
import ConfDefault from "./config/ConfigDefault.vue";
...
export default Vue.extend({
  ...
  created() {
    ...
    // `tagOptions` is reassigned below (the "VS" tag) with a different shape
    // (adds bold/italic, drops x/y) - `any` keeps that dynamic reuse legal.
    var tagOptions: any = { tag: this.$t('title.player', {nb: 'X'}), x: 0, y: 20, size: 40, color: "#000000", index: 4 };
  ...
  mounted() {
    ...
    (document.getElementById("can") as any).fabric = this.canvas;

    // apply the initial width-based zoom immediately on mount, since the
    // window's `load` event has typically already fired by the time this
    // component is created (see resizeCanvas(), which also handles this
    // on subsequent window resizes)
    this.resizeCanvas();
    ...
    download_file(name, contents, mime_type) {
        ...
        dlink.onclick = function() {
            ...
            // (`this` in a DOM `onclick` handler types as GlobalEventHandlers,
            // not the specific element, hence the cast)
    ...
    importJSON(file) {
      ...
      reader.onload = function(event) {
        try {
          var jsonObj = JSON.parse(event.target.result as string);

          self.importInfos(jsonObj);
        } catch (err) {
          console.error(err);
          self.$toasted.show('Could not import file: not a valid thumbnail export.', {
            icon: {
