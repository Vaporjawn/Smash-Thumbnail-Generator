class Character {
    ...
    toJSON() {
        var item: any = {};
        // NOTE: `id` is not a real accessor on this class (only `_id` is set in the
        // constructor) - this was already the case before the TS conversion, so
        // `item.id` has always evaluated to `undefined` here. Preserved as-is to
        // keep behavior identical; casting to `any` just satisfies the compiler.
        item.id = (this as any).id;
        item.game = this.game;
        item.name = this.name;
