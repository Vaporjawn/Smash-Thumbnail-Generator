...
class CustomRect {
    ...
    toJSON() {
        var item: any = {};
        // NOTE: there is no `index` getter on this class (only `_index`), so this
        // has always serialized as `undefined` - preserved as-is, see Character.ts
        // for the same pattern.
        item.index = (this as any).index;
