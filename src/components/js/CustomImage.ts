...
class CustomImage {
	...
	// NOTE: the async load callbacks below actually read/write `_imageShadowLoaded`
	// (missing the "g" in "Img"), a distinct property from `_imgShadowLoaded` above.
	// That mismatch predates this TS conversion; declaring the field here preserves
	// the existing (if accidental) behavior instead of silently "fixing" it.
	_imageShadowLoaded;
	shadowActive = true;
	_shadowX = -10;
	_shadowY = 10;
	...
	addImg() {
		...
		fabric.Image.fromURL(newUrl, function (oImg) {
			...
			// (2nd arg was already unused by addImgToGroup before this conversion)
			self.addImgToGroup(oImg)
